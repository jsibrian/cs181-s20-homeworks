...
\begin{enumerate}
    ...
    \includegraphics[scale=.40]{hw2/P1_Pics/figure_1-2.png}
    \item
    As we change the basis from basis 1 to basis 3 the variance goes up, tending towards overfitting. It is hard to determine the bias of any given basis since we use the same training data for every iteration of the program. But between models we see the bias decreases as we move from Basis 1 to Basis 3. This is especially apparent on the left side of the graph of Basis 3.
    \item
    The variance would decrease. This is because when given more information, data points, the model  would most likely adapt a more generalizable model because the data cannot be perfectly fit. Thus the output will change less drastically relative to the input resulting in lower variance.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \hspace{2cm} \newline
        ...
        \includegraphics[scale=.40]{hw2/P3_Pics/nolamsmall.png}
        
        $\eta = 0.01$ and $\lambda = 0.00001$.
        Smaller $\eta$ makes for a slower and smoother convergence. If $\eta$ is too large you see radical jumps as the optimal value is overshot. On the other hand a larger $\eta$ leads to a quicker convergence. That is why my value is reasonable as it is small enough to allow for smooth convergence and large enough to do so reasonably quick.
        
        As for $\lambda$ when it was larger the convergence happened much quicker but the loss was much larger than when lambda was equal to $0$. This makes sense since $\lambda$ always increase overall loss and makes it so that loss cannot be minimized as much which allows for faster convergence. A larger $\lambda$ also helps avoid overfitting. When $\lambda$ is small there is less of a penalty to the overall loss resulting in more time until convergence, but the chance of overfitting. 
        
        Given the data I went with a small $\lambda$ to avoid overfitting ever so slightly. Since when $\lambda = 0$ there did not seem to be too much overfitting going on. So it was a reasonable choice as it adds a bit of regularization without over-penalizing the loss.
    \item
        Separate Covariance negative log-likelihood: 107.856648944
        
        Shared Covariance negative log-likelihood: 126.063690098
        
        The separate covariance model has a lower loss. This is logical since the separate convariance model is not restricted to using linear boundaries between the classes and nonlinear boundaries serve as a better fit for the given data. The issue is that the separate covariance model might overfit the data and not generalize well due to the very unique boundaries calculated.
